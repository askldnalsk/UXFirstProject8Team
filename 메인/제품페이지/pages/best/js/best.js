(function(){ 
    ...
    const $best_steamed = localStorage.getItem('best_Steamed')

    const $item=document.querySelector('.item');

    const BestHTML = $item.innerHTML
        console.log(BestHTML);
        console.log($bun);
        window.localStorage.setItem('best',BestHTML)
    ...
    var $bun=[]
    var object =[]
    if($best_steamed!==null){
        $bun = $best_steamed.split(",");
        $bun.forEach((num1)=>{
    ...
    function test(bun){
        ...
        var Bvun=$bun.join();
        console.log(Bvun);
        window.localStorage.setItem('best_Steamed',Bvun)
    }
})();