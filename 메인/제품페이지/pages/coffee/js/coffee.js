(function(){ 
    ...
    const $ciffee_steamed = localStorage.getItem('ceffee_Steamed')

    const $item=document.querySelector('.item');

    const CoffeeHTML = $item.innerHTML
        console.log(CoffeeHTML);
        console.log($bun);
        window.localStorage.setItem('coffee',CoffeeHTML)
    ...
    var $bun=[]
    var object =[]
    if($ciffee_steamed!==null){
        $bun = $ciffee_steamed.split(",");
        $bun.forEach((num1)=>{
    ...
    function test(bun){
        ...
        var Bvun=$bun.join();
        console.log(Bvun);
        window.localStorage.setItem('ceffee_Steamed',Bvun)
    }
})();