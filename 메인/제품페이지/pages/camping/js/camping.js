(function(){ 
    ...
    const $camping_steamed = localStorage.getItem('camping_Steamed')

    const $item=document.querySelector('.item');

    const CampingHTML = $item.innerHTML
        console.log(CampingHTML);
        console.log($bun);
        window.localStorage.setItem('camping',CampingHTML)
    ...
    var $bun=[]
    var object =[]
    if($camping_steamed!==null){
        $bun = $camping_steamed.split(",");
        $bun.forEach((num1)=>{
    ...
    function test(bun){
        ...
        var Bvun=$bun.join();
        console.log(Bvun);
        window.localStorage.setItem('camping_Steamed',Bvun)
    }
})();